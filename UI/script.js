...
function renderAssignments() {
  ...
  visibleAssignments.forEach((a) => {
    ...
    li.innerHTML = `
      ...
      <div class="actions">
        <button type="button" class="js-extend student-only">Продлить срок</button>
        <button type="button" class="js-submit student-only">Отправить на проверку</button>
        <button type="button" class="js-grade teacher-only">Оценить</button>
        <button type="button" class="js-return teacher-only">Вернуть</button>
        <button type="button" class="js-delete teacher-only">Удалить</button>
      </div>
    `;
    list.appendChild(li);
  });
...
function handleAssignmentsClick(event) {
  ...
  const isResponsibleTeacher = isTeacher && (currentUser.role === 'admin' || findGroup(assignment.groupId)?.teacherId === currentUser.id);
  ...
  if (btn.classList.contains('js-return')) {
    assignment.status = 'В работе';
    assignment.comment = 'Требуется доработка';
    saveAssignments();
    renderAssignments();
  }
