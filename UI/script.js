...
function restoreDisciplines() {
  ...
  try {
    ...
    const fallback = JSON.parse(JSON.stringify(BASE_DISCIPLINES));
    localStorage.setItem(STORAGE_KEYS.disciplines, JSON.stringify(fallback));
    return fallback;
  }
}
...
function restoreUsers() {
  ...
  if (!raw) {
    localStorage.setItem(STORAGE_KEYS.users, JSON.stringify(BASE_USERS));
    return normalizeUsers([...BASE_USERS]);
  }
  try {
    const parsed = JSON.parse(raw);
    return Array.isArray(parsed) ? normalizeUsers(parsed) : normalizeUsers([...BASE_USERS]);
  } catch (e) {
...
function normalizeUsers(list) {
  return list.map((user) => ({
    ...
    completedCourses: 0,
    ...user,
    completedDisciplineIds: Array.isArray(user.completedDisciplineIds)
      ? [...user.completedDisciplineIds]
      : [],
    completedCourses: Array.isArray(user.completedDisciplineIds)
      ? user.completedDisciplineIds.length
...
function bootstrapAssignments() {
  ...
  studentUsers.forEach((student) => {
    ...
    prepared.push({
      id: `${student.id}-${group.id}-${Date.now()}-${Math.random()}`,
      title: tpl.title,
...
function getGroupStudentIds(groupId) {
  return users.filter((u) => u.groupId === groupId).map((u) => u.id);
}
...
function syncGroupsWithUsers() {
  disciplines.forEach((d) => {
    d.groups.forEach((g) => {
      g.students = getGroupStudentIds(g.id);
    });
  });
}
...
function renderCatalog() {
  ...
  disciplines.filter((d) => !selectedId || d.id === selectedId).forEach((discipline) => {
    ...
    const seats = discipline.groups.map((g) => 10 - getGroupStudentIds(g.id).length);
...
function renderEnrollForm(prefilledDisciplineId) {
  ...
  function updateGroupOptions() {
    ...
    currentDiscipline.groups.forEach((g) => {
      const free = 10 - getGroupStudentIds(g.id).length;
      const opt = document.createElement('option');
      opt.value = g.id;
...
function renderAssignments() {
  ...
  visibleAssignments.slice(0, assignmentsVisibleCount).forEach((a) => {
    ...
    if (isOwnerStudent && a.status === 'В работе') {
      ...
      actions.push('<button type="button" class="js-student-extend">Запросить продление</button>');
    }
    ...
    if (teacherList) {
      ...
      disciplines.forEach((d) => {
        d.groups
          ...
          .forEach((g) => {
            const li = document.createElement('li');
            const students = getGroupStudentIds(g.id)
...
function handleResultFormSubmit(event) {
  event.preventDefault();
  alert('Изменения сохранения результатов доступны через реальные задания.');
}
...
function handleEnrollSubmit(event) {
  ...
  const studentIds = getGroupStudentIds(group.id);
...
function issueAssignmentsForStudent(student, group, discipline) {
  const templates = TASK_TEMPLATES[discipline.id] || [];
  const hasTemplates = assignments.some(
    (a) => a.studentId === student.id && a.disciplineId === discipline.id && typeof a.templateIndex === 'number',
  );
...
function releaseNextTemplateForStudent(studentId, groupId, disciplineId, fromIndex) {
  const templates = TASK_TEMPLATES[disciplineId] || [];
  const nextIndex = fromIndex + 1;
  if (nextIndex >= templates.length) return;
  const alreadyExists = assignments.some(
    (a) => a.studentId === studentId && a.disciplineId === disciplineId && a.templateIndex === nextIndex,
  );
  if (alreadyExists) return;
  const previousIncomplete = assignments.some(
    (a) =>
      a.studentId === studentId &&
      a.disciplineId === disciplineId &&
      typeof a.templateIndex === 'number' &&
      a.templateIndex < nextIndex &&
      a.status !== 'Проверено',
  );
  if (previousIncomplete) return;
  ...
  assignmentsModel.addObj({
    ...
    title: tpl.title,
    description: tpl.description,
    discipline: findDisciplineByGroup(groupId)?.title || disciplineId,
    disciplineId,
    groupId,
    studentId,
    status: 'В работе',
...
function refreshCourseCompletion(studentId, disciplineId) {
  const user = findUserById(studentId);
  if (!user) return;
  const templates = TASK_TEMPLATES[disciplineId] || [];
  const templateAssignments = assignments.filter(
    (a) => a.studentId === studentId && a.disciplineId === disciplineId && typeof a.templateIndex === 'number',
  );
  const isCompleted =
    templates.length > 0 &&
    templates.every((_, idx) => templateAssignments.some((a) => a.templateIndex === idx && a.status === 'Проверено'));
  user.completedDisciplineIds = Array.isArray(user.completedDisciplineIds) ? user.completedDisciplineIds : [];
  if (isCompleted && !user.completedDisciplineIds.includes(disciplineId)) {
    user.completedDisciplineIds.push(disciplineId);
  }
  if (!isCompleted && user.completedDisciplineIds.includes(disciplineId)) {
    user.completedDisciplineIds = user.completedDisciplineIds.filter((id) => id !== disciplineId);
  }
  user.completedCourses = user.completedDisciplineIds.length;
  if (currentUser?.id === user.id) {
    currentUser = { ...currentUser, completedCourses: user.completedCourses };
    saveCurrentUser();
  }
  saveUsers();
}

function recalculateCourseCompletions() {
  users = normalizeUsers(users);
  users.forEach((user) => {
    if (user.role !== 'student') return;
    const studentAssignments = assignments.filter(
      (a) => a.studentId === user.id && typeof a.templateIndex === 'number',
    );
    const disciplinesWithAssignments = Array.from(new Set(studentAssignments.map((a) => a.disciplineId)));
    disciplinesWithAssignments.forEach((disciplineId) => {
      const templates = TASK_TEMPLATES[disciplineId] || [];
      const completed =
        templates.length > 0 &&
        templates.every((_, idx) =>
          studentAssignments.some((a) => a.disciplineId === disciplineId && a.templateIndex === idx && a.status === 'Проверено'),
        );
      if (completed && !user.completedDisciplineIds.includes(disciplineId)) {
        user.completedDisciplineIds.push(disciplineId);
      }
      if (!completed) {
        user.completedDisciplineIds = user.completedDisciplineIds.filter((id) => id !== disciplineId);
    ...
    user.completedDisciplineIds = Array.from(new Set(user.completedDisciplineIds));
    user.completedCourses = user.completedDisciplineIds.length;
  });
  saveUsers();
  if (currentUser) {
    ...
    if (found) {
      currentUser = { ...currentUser, completedCourses: found.completedCourses };
      saveCurrentUser();
    }
  }
}
...
function handleAssignmentsClick(event) {
  ...
  if (btn.classList.contains('js-student-extend')) {
    if (!currentUser || currentUser.role !== 'student' || assignment.studentId !== currentUser.id) {
      alert('Продление может запросить только владелец задания');
      return;
    }
    if (assignment.studentExtendRequested) {
      alert('Продление уже выполнено');
      return;
    }
    assignment.deadline = new Date(assignment.deadline.getTime() + 2 * 24 * 60 * 60 * 1000);
    assignment.studentExtendRequested = true;
    saveAssignments();
    renderAssignments();
    alert('Срок продлён на 2 дня по запросу студента');
  }
  ...
  if (btn.classList.contains('js-grade')) {
    ...
    if (typeof assignment.templateIndex === 'number') {
      ...
      releaseNextTemplateForStudent(
        assignment.studentId,
        assignment.groupId,
        assignment.disciplineId,
  ...
  if (btn.classList.contains('js-return')) {
    ...
    assignment.studentExtendRequested = false;
    saveAssignments();
    refreshCourseCompletion(assignment.studentId, assignment.disciplineId);
    renderAssignments();
    renderProfile();
...
function handleAdminAddSubmit(event) {
  ...
  if (role === 'student') {
    ...
    newUser.completedDisciplineIds = [];
  }
  users.push(newUser);
...
function handleAdminCourseSubmit(event) {
  ...
  if (teacherId && !users.some((u) => u.id === teacherId && u.role === 'teacher')) {
  ...
  if (findGroup(groupId)) {
    alert('Не удалось сгенерировать уникальный идентификатор группы, попробуйте снова');
    return;
  }
  const newDiscipline = {
    id,
...
function guardRoute() {
  const allowed = ['#welcome', '#login', '#register', '#home', '#enroll', ''];
...
function init() {
  ...
  document.querySelector('#result form')?.addEventListener('submit', handleResultFormSubmit);
